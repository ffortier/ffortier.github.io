...
void fprint_usage(FILE *fd, const char *program)
{
    ...
    fprintf(fd, "\tdft <input_signal.dat> <rex.dat> <imx.dat> <mag.dat>\n");
...
int dft_command(int argc, char **argv, const char *program)
{
    ...
    const char *mag_path = shiftarg(&argc, &argv);
    ...
    SignalBuffer mag = signal_dft_magnetude(freq.rex.items, freq.imx.items, freq.rex.count);

    signal_write_data(rex_path, freq.rex.items, freq.rex.count);
    signal_write_data(imx_path, freq.imx.items, freq.imx.count);
    signal_write_data(mag_path, mag.items, mag.count);
